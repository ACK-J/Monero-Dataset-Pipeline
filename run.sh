...
EOL
		...
		xfce4-terminal --tab -x /bin/bash -c "i=1; while : ;do ./${walletName}-spend.exp \$(python3 -c \"import random;print(format(random.uniform(0.0001, 0.000000000001), '.12f'))\") \`python3 -c \"import random;print(random.choices([0,1,2,3,4], weights=[18,18,18,18,10], k=1)[0])\"\`; date; echo -en '\033[34mNumber of successful transactions: \033[0m'; echo \$i; ((i++)); python3 ../../Gamma.py; done"
		sleep 60
